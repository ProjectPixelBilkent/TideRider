...
public class FlameProjectile : Bullet
{
    ...
    private float perFlameDamageMultiplier = 0.15f;
    private int maxDamageTicks = 1;
    private int damageTickCount;

    public void SetFlight(float coneLength, float flameLifetime)
    {
        lifetime = Mathf.Max(0.05f, flameLifetime);
    ...
    public void SetDamageConfig(float cooldownSeconds, int maxTicks, float damageMultiplier)
    {
        ...
        perFlameDamageMultiplier = Mathf.Max(0f, damageMultiplier);
    }
    ...
    protected override void FixedUpdate()
    {
        ...
        rigidBody.linearVelocity = direction * travelSpeed + shipSpeed;
        transform.Rotate(0f, 0f, -spinSpeed * Time.fixedDeltaTime);
    }

    protected override void OnCollisionEnter2D(Collision2D collision)
    {
        TryApplyFlameDamage(collision.collider);
    }
    ...
    private void TryApplyFlameDamage(Collider2D targetCollider)
    {
        ...
        int damageToApply = Mathf.Max(1, Mathf.RoundToInt(WeaponLevel.damage * perFlameDamageMultiplier));
